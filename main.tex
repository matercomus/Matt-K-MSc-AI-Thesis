...
The central goal is enabling high-utility trajectory data release without revealing private information about individuals~\cite{buchholzSystematisationKnowledgeTrajectory2024,raoLSTMTrajGANDeepLearning2020a,liuTrajGANsUsingGenerative,jinSurveyExperimentalStudy2023,maTrajectoryPrivacyProtection2021,naghizadePrivacyContextawareRelease2020}. This would allow researchers to develop and test anomaly detection systems without requiring direct access to sensitive real-world data~\cite{buchholzSystematisationKnowledgeTrajectory2024,raoLSTMTrajGANDeepLearning2020a,liuTrajGANsUsingGenerative}.
...
The ultimate aim of generating synthetic trajectory datasets is to replace original trajectories for data sharing and publication~\cite{buchholzSystematisationKnowledgeTrajectory2024,raoLSTMTrajGANDeepLearning2020a,liuTrajGANsUsingGenerative}. This directly addresses the need for anomaly detection research in urban taxi operations to proceed without requiring access to sensitive real-world data. Such an approach would overcome privacy concerns and regulatory hurdles associated with using actual mobility traces~\cite{buchholzSystematisationKnowledgeTrajectory2024,raoLSTMTrajGANDeepLearning2020a,liuTrajGANsUsingGenerative}. Synthetic data must support diverse analytical tasks including spatial and temporal analyses, classification, clustering, and anomaly detection while maintaining utility for research purposes~\cite{raoLSTMTrajGANDeepLearning2020a,chenDifferentiallyPrivateTrajectory2011}. The CC-Net system demonstrates privacy-preserved taxi demand prediction, achieving high accuracy while ensuring privacy by design~\cite{ozekiBalancingPrivacyUtility2023}.
...
\begin{compactoutline}
  \outlineitem{LibCity Framework Integration -- Implementation of classification as extension to LibCity}
  \outlineitem{Algorithm Implementation -- isolation forest adaptation for trajectory data}
  \outlineitem{Key Adaptations for Trajectory Data -- Feature engineering and distance metrics}
  \outlineitem{Framework Compatibility -- Ensuring anomaly detection works with LibCity data formats}
\end{compactoutline}

\subsubsection{Statistical Pattern Extraction}
...
\subsubsection{Enhanced Anomaly Detection Framework}
\label{sec:improve}
...
\subsubsection{Indicators Discussion}
\label{sec:indicators}

\begin{compactoutline}
  \outlineitem{Anomaly Detection Indicators -- Performance metrics and evaluation criteria}
  \outlineitem{Visualized Examples -- Concrete examples of detected anomalies}
  \outlineitem{Indicator Interpretation -- Understanding and validating detection results}
\end{compactoutline}
...
\begin{compactoutline}
  \outlineitem{Computational Efficiency -- Runtime and memory performance analysis}
  \outlineitem{Scalability Analysis -- Performance with varying dataset sizes}
  \outlineitem{Visualized Examples -- Efficiency comparisons and bottleneck analysis}
\end{compactoutline}
...
\label{sec:generation-framework}

\begin{compactoutline}
  \outlineitem{LibCity Framework Extension -- Implementation of data generation as extension to LibCity}
  \outlineitem{Statistical Model Architecture -- Core generation approach and implementation}
  \outlineitem{Graph-Based Trajectory Generation -- Forms graphs methodology}
  \outlineitem{Pattern Preservation -- Maintaining essential trajectory characteristics}
  \outlineitem{Dataset Compatibility -- Ensuring generated data works with LibCity's existing datasets}
\end{compactoutline}

\subsubsection{Privacy Concerns and Protection}
\label{sec:privacy-concerns}

\begin{compactoutline}
  \outlineitem{Privacy Risk Assessment -- Identification of privacy vulnerabilities}
  \outlineitem{Privacy Preservation Mechanisms -- Differential privacy, k-anonymity integration}
  \outlineitem{Worst Case Scenario Analysis -- Privacy implications of synthetic data creation}
\end{compactoutline}

\subsubsection{Comparison and Preference}
\label{sec:comparison}

\begin{compactoutline}
  \outlineitem{Approach Comparison -- Evaluation of different generation methods}
  \outlineitem{Preference Justification -- Rationale for chosen methodology}
  \outlineitem{Trade-off Analysis -- Privacy vs. utility considerations}
\end{compactoutline}
...
\begin{compactoutline}
  \outlineitem{Validation Metrics -- Statistical and behavioral validation methods}
  \outlineitem{Testing Procedures -- Systematic evaluation of synthetic data quality}
  \outlineitem{Utility Preservation -- Ensuring research value of synthetic datasets}
\end{compactoutline}
...
\begin{compactoutline}
  \outlineitem{Data Volume Analysis -- Daily data sizes, total trajectory counts}
  \outlineitem{Basic Statistical Properties -- Trip durations, distances, spatial coverage}
  \outlineitem{Temporal Distribution -- Time-of-day patterns, weekly variations}
  \outlineitem{Spatial Coverage -- Geographic extent, density distributions}
\end{compactoutline}
...
\begin{figure}[h]
\centering
% TODO: Add data preprocessing flowchart here
\fbox{\parbox{0.8\textwidth}{\centering \vspace{2cm} \textbf{Data Preprocessing Flowchart} \\ \textit{(To be added)} \vspace{2cm}}}
\caption{Data preprocessing pipeline flowchart showing the complete workflow from raw GPS data to LibCity-standardized trajectory datasets.}
\label{fig:preprocessing-flowchart}
\end{figure}
...
\begin{compactoutline}
  ...
  \outlineitem{LibCity Format Standardization -- Converting data to LibCity's standardized format for compatibility}

  To ensure standardization and reproducibility, this research leverages the LibCity framework~\cite{wangEfficientComprehensiveUrban2023}, which provides standardized data formats and comprehensive tools for spatio-temporal data analysis. The LibCity framework enables seamless integration with multiple datasets and facilitates the implementation of classification and data generation extensions while maintaining compatibility with existing prediction tasks and evaluation frameworks.
...
\begin{compactoutline}
  \outlineitem{LibCity Integration -- Leveraging LibCity's evaluation tools and result visualization capabilities}
  ...
  \outlineitem{Cross-Dataset Validation -- Testing with multiple LibCity-compatible datasets}
\end{compactoutline}
...
\begin{compactoutline}
  \outlineitem{LibCity Benchmark Evaluation -- Leveraging LibCity's comprehensive evaluation framework}
  \outlineitem{Cross-Model Comparison -- Comparing synthetic data quality against other LibCity models}
  \outlineitem{Multi-Task Performance -- Running all relevant LibCity benchmarks and prediction tasks}
  ...
  \outlineitem{Standardized Evaluation Metrics -- Using LibCity's established evaluation protocols}
\end{compactoutline}
...
\begin{compactoutline}
  ...
  \outlineitem{Framework Extension -- Classification and data generation extensions to LibCity framework}
  \outlineitem{Standardization Benefits -- Enabling cross-dataset compatibility and reproducible research}
\end{compactoutline}
