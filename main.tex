...
\begin{algorithmic}[1]
\Require
$D_{real}$: Dataset of real normal trajectories.
$N$: Number of refinement iterations.
$M$: Number of trajectories to generate per iteration.
$Detector$: Unsupervised anomaly detection model.
$Rules$: Heuristic rules for anomaly categorization.
\Ensure
$G_N$: Final generative model for conditional anomaly generation.
$D_{anomalous}$: Labeled dataset of synthetic anomalies.
...
\For{$i \gets 1$ to $N$}
    \State $D_{new\_anomalies} \gets \text{MineAnomalies}(G_{i-1}, M, Detector, Rules)$. \Comment{See Algorithm~\ref{alg:anomaly-mining}}
...
\begin{algorithmic}[1]
\Require
$G_{in}$: Input generative model.
$M$: Number of trajectories to generate.
$Detector$: Unsupervised anomaly detection model.
$Rules$: Heuristic rules for anomaly categorization.
\Ensure
$D_{new\_anomalies}$: Labeled dataset of new synthetic anomalies.
...
\begin{description}
    \item[Unsupervised Anomaly Detection] A lightweight anomaly detection algorithm (such as Isolation Forest, Autoencoder, or One-Class SVM) is applied to the \texttt{synthetic\_normal} dataset to identify outliers without requiring pre-existing labels.
    \item[Rule-Based Curation and Categorization] The detected \texttt{potential\_anomalies} are filtered and classified into interpretable categories (e.g., speeding, off-road, unusual stops) using heuristic rules based on kinematic and spatial properties.
...
\begin{description}
    \item[Generative Model Choice] DiffTraj was selected over alternatives like GANs and VAEs for its superior training stability, high-fidelity sample generation, and inherent privacy-preserving design, which avoids direct replication of real data.
...
\begin{description}
    \item[Key Performance Metrics] Evaluation focuses on Precision, Recall, and the F1-Score, which provide a balanced view of the detector's ability to correctly identify rare anomalous instances.
...
\begin{description}
    ...
    \item[Statistical Fidelity] Distribution comparison tests (e.g., Kolmogorov-Smirnov, Jensen-Shannon divergence) are used to quantitatively measure the statistical similarity between the real and synthetic trajectory data distributions for key properties like trip duration and distance.
    \item[Downstream Task Performance] The utility of the synthetic data is further validated by evaluating the performance of downstream models (e.g., travel time estimation, destination prediction) trained on the synthetic data versus models trained on real data, leveraging the LibCity framework's benchmark tasks.
\end{description}
