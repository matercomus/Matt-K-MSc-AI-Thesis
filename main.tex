...
\begin{description}
    ...
    In addition to traditional detectors, we employ TrajBERT~\cite{siTrajBERTBERTBasedTrajectory2024}, a BERT-based transformer model tailored for trajectory data. Each GPS trajectory is first discretized into a sequence of region IDs (e.g., geohash cells). TrajBERT is trained in a self-supervised manner to predict masked regions within these sequences, learning typical spatial-temporal mobility patterns without requiring anomaly labels. For anomaly detection, we follow a reconstruction-based approach: for each trajectory, a subset of tokens is randomly masked, and TrajBERT is used to predict the masked tokens. The reconstruction loss for each masked token (e.g., cross-entropy between the predicted and true token) is computed, and these per-token losses are then aggregated—typically by taking the mean—to yield a single anomaly score for the trajectory. Trajectories with higher mean reconstruction loss are considered more anomalous, as they deviate more from the learned normal patterns~\cite{siTrajBERTBERTBasedTrajectory2024}. This approach leverages the model’s ability to capture complex, context-dependent movement patterns and is robust to data sparsity and irregular sampling. The top-ranked anomalies are then passed to the diverse querying and rule-based curation steps for manual review and categorization.
