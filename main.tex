...
\begin{description}
    \item[Unsupervised Anomaly Detection] A lightweight anomaly detection algorithm (such as Isolation Forest, Autoencoder, or One-Class SVM) is applied to the \texttt{synthetic\_normal} dataset to identify outliers without requiring pre-existing labels. Modern diffusion-based anomaly detection frameworks like DiffTAD~\cite{liDiffTADDenoisingDiffusion2024} also use Isolation Forest as a baseline for performance comparison, highlighting its relevance.
    \item[Rule-Based Curation and Categorization] The detected \texttt{potential\_anomalies} are filtered and classified into interpretable categories using a taxonomy of heuristic rules based on kinematic and spatial properties. Thresholds for route length and travel time are set following the statistical approach of Wang et al.~\cite{wangStatisticalFrameworkTaxi2020,wangDetectingAnomalousTrajectories2018}, where a trajectory is considered anomalous if its length or duration exceeds the mean of normal trajectories for its origin-destination pair by more than an empirically determined margin (e.g., $L_\rho$ for length, $T_\rho$ for time). Specifically:
...
To quantify the impact of key components in the framework, we performed two ablation experiments:
\begin{itemize}
    \item \textbf{No Rule-Based Curation:} Removing the rule-based curation step and relying solely on the unsupervised detector led to a drop in anomaly detection F1-score from 0.81 to 0.74, and the proportion of interpretable anomalies decreased from 92\% to 61\%.
    \item \textbf{No Iterative Refinement:} Using a single-pass (non-iterative) approach reduced the number of unique anomaly categories generated from 6 to 3, and the overall anomaly detection F1-score dropped from 0.81 to 0.76.
\end{itemize}
These results demonstrate that both rule-based curation and iterative refinement are critical for achieving high anomaly interpretability, diversity, and detection performance.
