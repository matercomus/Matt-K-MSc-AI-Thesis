\begin{figure}[t]
    ...
    \begin{tikzpicture}[
        ...
        % Arrow from input to student - end at right edge
        \draw[arrow] (input.east) -- ++(0.8,0) |- (student.east);
        ...
        \node[data, above=1.2cm of mapping, yshift=4.8cm, minimum width=3cm] (temp) {
        ...
        \draw[arrow] (temp) -- (kl);
        % Ground truth to CE - connect to left edge with properly positioned rotated label
        \coordinate (gt_point) at ([xshift=-2.5cm]input.west);
        \coordinate (gt_vertical_mid) at ([xshift=-2.5cm, yshift=5cm]input.west);
        \draw[arrow, dashed] (input.west) -- (gt_point) -- (gt_vertical_mid) node[pos=0.5, rotate=90, above, font=\footnotesize] {labels} -- (gt_vertical_mid |- ce.west) -- (ce.west);
