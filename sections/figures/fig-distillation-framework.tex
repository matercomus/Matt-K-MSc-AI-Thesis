\begin{figure}[t]
    ...
    \begin{tikzpicture}[
        ...
        % Arrow from input to HOSER: Route right, then up to HOSER east edge
        \coordinate (input_to_student_mid) at ($(input.east)+(1.0,0)$);
        ...
        % Input → Cross-Entropy: Route left, up, then right to CE west edge
        % Clean vertical path with properly positioned label
        \coordinate (labels_left) at ($(input.west)+(-1.7,0)$);
        \coordinate (labels_up) at (labels_left |- ce.west);
        \draw[arrow, dashed] (input.west) -- (labels_left) 
            -- (labels_up) node[midway, rotate=90, above, font=\footnotesize] {labels} 
            -- (ce.west);
