\begin{figure}[t]
    ...
    \begin{tikzpicture}[
        ...
        \node[trainable, above=1cm of candidates, minimum width=2.8cm, minimum height=0.9cm, anchor=west] (student) {
        ...
        \draw[arrow] (candidates) -- (student.west);
        % Arrow from input to student - end at right edge with gentle bend
        \coordinate (input_to_student_mid) at ($(input.east)+(1.1,0)$);
        \draw[arrow] (input.east) -- (input_to_student_mid) |- (student.east);
        ...
        % 6. Temperature scaling - reduced spacing
        \node[data, above=1.2cm of mapping, yshift=4.3cm, minimum width=3cm] (temp) {
        ...
        % 7. Multi-task losses
        \node[loss, above left=1.0cm and 1.3cm of temp] (ce) {
            \textbf{Cross-Entropy}
        };
        \node[loss, above=1.0cm of temp] (kl) {
            \textbf{KL Divergence}
        };
        \node[loss, above right=1.0cm and 1.3cm of temp] (time_loss) {
            \textbf{Time Loss}
        };
        
        \draw[arrow] (temp) -- (kl);
        % Ground truth to CE - clean routed path with rotated label
        \coordinate (labels_left) at ($(input.west)+(-1.6,0)$);
        \coordinate (labels_up) at ($(labels_left)+(0,4.8)$);
        \coordinate (ce_left) at ($(ce.west)+(-0.35,0)$);
        \draw[arrow, dashed]
            (input.west) -- (labels_left)
            -- (labels_up) node[pos=0.6, rotate=90, above, font=\footnotesize] {labels}
            -- (ce_left) -- (ce.west);
        % Student output to time loss - end at right edge
        \draw[arrow, dashed] (student_out.east) -- ++(0.7,0) |- (time_loss.east);
        
        % 8. Total loss
        \node[loss, above=1.0cm of kl, minimum width=4.5cm, minimum height=0.8cm] (total) {
            \textbf{Total Loss}\\
