...
where $d_{\text{gc}}(\cdot, \cdot)$ is the great-circle distance in kilometers. We create histograms with 100 bins spanning $[0, \max(\mathcal{D}_{\text{real}})]$ where $\mathcal{D}_{\text{real}}$ is the set of all real trajectory distances, plus one bin for $[\max(\mathcal{D}_{\text{real}}), \infty)$. JSD is computed between normalized histograms of real and generated distance distributions.

\paragraph{Per-Segment Duration Distribution}

For trajectories with timestamps $T = \{(r_1, t_1), (r_2, t_2), \ldots, (r_n, t_n)\}$, we extract \emph{per-segment} durations (not total trip duration):
...
where durations are measured in minutes. Each segment contributes one sample to the duration distribution. Histograms use 100 bins spanning $[0, \max(\mathcal{T}_{\text{real}})]$ plus infinity bin. JSD compares normalized histograms of all segment durations from real and generated trajectories.
...
Histograms use 100 bins spanning $[0, \max(\mathcal{R}_{\text{real}})]$ plus infinity bin. Lower JSD values indicate proper spatial complexity modeling.
