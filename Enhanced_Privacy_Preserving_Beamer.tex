...
\begin{frame}{Privacy-by-Design Framework}
  \begin{center}
    \begin{tikzpicture}[scale=0.8]
      % Two layers only, focus on sampling
      \draw[fill=LightGray, draw=ScienceBlue, thick, rounded corners] (0,2) rectangle (10,3.5);
      \node at (5,2.75) {\large\textbf{Layer 1: Trajectory-Level Protection}};
      \draw[fill=LightGray, draw=VUBlue, thick, rounded corners] (0,0) rectangle (10,1.5);
      \node at (5,0.75) {\large\textbf{Layer 2: Noise-Based Decoupling}};
      % Privacy notes
      \node[right] at (10.2,2.75) {\footnotesize Entire trajectory as privacy unit};
      \node[right] at (10.2,0.75) {\footnotesize Generation from random noise};
    \end{tikzpicture}
  \end{center}
  \vspace{1em}
